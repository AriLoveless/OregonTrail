...
OregonTrail.Event.eventTypes = [
  {
    type: 'STAT-CHANGE',
    notification: 'negative',
    stat: 'crew',
    value: -3,
    text: 'Your crew has dysentery. Casualties: '
  },
  {
    type: 'STAT-CHANGE',
    notification: 'negative',
...
OregonTrail.Event.generateEvent = function(){
  //pick random one
...
OregonTrail.Event.stateChangeEvent = function(eventData) {
...
OregonTrail.Event.shopEvent = function(eventData) {
...
OregonTrail.Event.attackEvent = function(eventData){
  var firepower = Math.round((0.7 + 0.6 * Math.random()) * OregonTrail.ENEMY_FIREPOWER_AVG);
  var gold = Math.round((0.7 + 0.6 * Math.random()) * OregonTrail.ENEMY_GOLD_AVG);
